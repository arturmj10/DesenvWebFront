...
function ProdutosPage() {
  ...
  const handleSalvar = async (produto) => {
    try {
      if (produtoEditando) {
        ...
        setProdutos(prev =>
          prev.map(prod => prod.id === produto.id ? produto : prod)
        );

        toast.success(`Produto "${produto.nome}" atualizado com sucesso!`);
      } else {
        const novoProduto = await criarProduto(produto);
        const categoriaDoProduto = categorias.find(c => c.id === novoProduto.categoriaId);
  ...
  const handleDeletar = async () => {
    try {
      ...
      setProdutos(prev =>
        prev.filter(prod => prod.id !== produtoDeletando.id)
      );
